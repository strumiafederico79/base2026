...
def detect_key(
    ...
    try:
        ...
        # 1.0 sería concentración extrema en 7 pitch classes.
        confidence = float(np.clip((best_score - 7 / 12) / (1 - 7 / 12), 0.0, 1.0))
...
def apply_time_varying_pitch_shift(
    ...
    if window_length < 1024:
        return apply_pitch_shift(audio, sr, float(np.nanmedian(shifts_cents)))
    ...
    if audio.ndim == 1:
        ...
        processed = np.where(weight > 1e-6, processed, audio)
    else:
        processed = np.divide(
            out,
            np.maximum(weight, 1e-6),
        ...
        processed = np.where(weight[np.newaxis, :] > 1e-6, processed, audio)
...
class PitchCorrectionProcessor:
    ...
    def process(
        ...
        # BUGFIX: el frontend manda scale="" (string vacío) para "detección
        # automática", no None — pero la validación de acá abajo y el chequeo
        # de "scale is None" más adelante solo reconocían None como auto.
        # Con "" llegaba hasta el `scale not in SCALES` y explotaba con
        # ValueError en cada corrección con "Automática" seleccionado (la
        # opción por default), silenciada por el try/except del caller.
        scale = scale or None
