...
def iter_mastering_chunks(audio: np.ndarray, sr: int,
    ...
    ola_tail2 = None
    ola_samples2 = max(0, int(OLA_CROSSFADE_SECONDS * sr))
    ola_samples2 = min(ola_samples2, chunk_samples // 2)
    if ola_samples2 > 0:
        t2 = np.linspace(0.0, np.pi / 2.0, ola_samples2, dtype=np.float32)
        ola_fade_out2 = np.cos(t2)
        ola_fade_in2  = np.sin(t2)
    ...
    for i in range(n_chunks):
        ...
        # ── OLA crossfade coseno entre chunks ─────────────────────────────────
        if ola_samples2 > 0 and ola_tail2 is not None and out_block.shape[-1] >= ola_samples2:
            cf = min(ola_samples2, ola_tail2.shape[-1])
            out_block[:, :cf] = (ola_tail2[:, -cf:] * ola_fade_out2[-cf:][np.newaxis, :]
                                 + out_block[:, :cf] * ola_fade_in2[-cf:][np.newaxis, :])
        if ola_samples2 > 0 and out_block.shape[-1] >= ola_samples2:
            ola_tail2 = out_block[:, -ola_samples2:].copy()
        else:
            ola_tail2 = None
        ...
        metrics = {
            ...
            "mb_meters":      chain_meters.get("mb", {}),
            "comp_meters":    chain_meters.get("comp", {}),
            "parallel_meters": chain_meters.get("parallel", {}),  # BUGFIX: faltaba — meter de comp paralela nunca llegaba al frontend
            "glue_meters":    chain_meters.get("glue", {}),
            "dyneq_meters":   chain_meters.get("dyneq", {}),
            "reso_meters":    chain_meters.get("reso", {}),
            "ms_comp_meters": chain_meters.get("ms_comp", {}),
            "tonal_balance_meters": chain_meters.get("tonal_balance", {}),  # BUGFIX: faltaba — tonal balance meter no llegaba al frontend
            "pre_limiter":    chain_meters.get("pre_limiter", {}),
            "post_limiter":   chain_meters.get("post_limiter", {}),
            "spectrum": spectrum_data,
            "dynamic_eq_recommendation": dynamic_eq_recommendation,
...
def master_stream_to_pcm16(audio: np.ndarray, sr: int,
    ...
    Con prefetch=3 (default) el servidor procesa los próximos N chunks en
    paralelo usando _CHAIN_POOL (ProcessPoolExecutor con _N_CHAIN_WORKERS
    procesos). Mientras el cliente reproduce el chunk actual, los siguientes
    ya están listos — elimina la latencia de espera entre chunks.
    ...
    # Prefetch paralelo: submit N chunks al pool, yield en orden
    # Usamos iter_mastering_chunks como fuente de bloques PRE-PROCESADOS
    # (con overlap/OLA ya resuelto) y los PCM se generan en paralelo.
    # Si el pool no está disponible, cae al serial.
    try:
